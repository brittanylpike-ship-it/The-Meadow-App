...
import {
  getFirstBloomRitualById,
  getFirstBloomRitualReturnState,
  getFirstBloomRitualSaveCopy,
  type FirstBloomRitualId
} from "@/features/memory/first-bloom-memory.mjs";
import { useMeadowState } from "@/features/world/use-meadow-state";

type FirstBloomRitualScreenProps = {
  ritualId: FirstBloomRitualId;
};

const firstBloomRitualSceneIds: Record<FirstBloomRitualId, MeadowSceneId> = {
  grounding: "ritual_grounding",
  opening: "ritual_opening",
  anchoring: "ritual_anchoring",
  emergence: "ritual_emergence",
  integration: "ritual_integration",
};

export function FirstBloomRitualScreen({ ritualId }: FirstBloomRitualScreenProps) {
  const ritual = getFirstBloomRitualById(ritualId);
  ...
  const returnState = useMemo(() => {
    ...
    return getFirstBloomRitualReturnState(meadow.state, ritualId);
  ...
  async function save() {
    await meadow.saveFirstBloomRitual.mutateAsync({
      ritualId,
      response,
      detail
    });
  ...
  return (
    <MeadowScreen title={ritual.title} subtitle={returnState?.message ?? `${ritual.title} is waiting in soft growth.`}>
      <MeadowSceneImage sceneId={firstBloomRitualSceneIds[ritualId]} accessibilityLabel={`A rendered ${ritual.title} ritual page`} />
      ...
      {authLoading || meadow.loading || !returnState ? (
        ...
        <>
          ...
          <MeadowPanel>
            ...
            />
            <MeadowButton
              label={getFirstBloomRitualSaveCopy(ritualId, meadow.saveFirstBloomRitual.isPending)}
              onPress={save}
              disabled={meadow.saveFirstBloomRitual.isPending}
            />
          </MeadowPanel>
