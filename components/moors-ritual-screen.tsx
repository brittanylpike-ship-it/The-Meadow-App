...
type MoorsRitualScreenProps = {
  ritualId: MoorsRitualId;
};

const moorsRitualSceneIds: Record<MoorsRitualId, MeadowSceneId> = {
  canopy_cloak: "ritual_canopy_cloak",
  mire: "ritual_mire",
  bramble: "ritual_bramble",
  fog: "ritual_fog",
  vanishing_path: "ritual_vanishing_path",
};

export function MoorsRitualScreen({ ritualId }: MoorsRitualScreenProps) {
  const ritual = getMoorsRitualById(ritualId);
  ...
  const returnState = useMemo(() => {
    ...
    return getMoorsRitualReturnState(meadow.state, ritualId);
  ...
  async function save() {
    await meadow.saveMoorsRitual.mutateAsync({
      ritualId,
      response,
      detail
    });
  ...
  return (
    <MeadowScreen title={ritual.title} subtitle={returnState?.message ?? `${ritual.title} is waiting in the fog.`}>
      <MeadowSceneImage sceneId={moorsRitualSceneIds[ritualId]} accessibilityLabel={`A rendered ${ritual.title} ritual page`} />
      ...
      {authLoading || meadow.loading || !returnState ? (
        ...
        <>
          ...
          <MeadowPanel>
            ...
            />
            <MeadowButton
              label={getMoorsRitualSaveCopy(ritualId, meadow.saveMoorsRitual.isPending)}
              onPress={save}
              disabled={meadow.saveMoorsRitual.isPending}
            />
          </MeadowPanel>
